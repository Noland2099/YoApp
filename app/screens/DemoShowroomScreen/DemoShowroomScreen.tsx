    ...
    return (
        <View>
            ...
            {item.useCases.map((u, index) => (
                <ListItem key={`section${sectionIndex}-${u}`} onPress={() => handleScroll?.(sectionIndex, index + 1)} text={u} rightIcon={isRTL ? "caretLeft" : "caretRight"} />
            ))}
        </View>
    );
};
...
export const DemoShowroomScreen: FC<DemoTabScreenProps<"DemoShowroom">> = function DemoShowroomScreen(_props) {
    ...
    useEffect(() => {
        if (params !== undefined && Object.keys(params).length > 0) {
            ...
            if (params.itemIndex) {
                try {
                    findItemIndex = demoValues[findSectionIndex].data({ themed, theme }).findIndex((u) => slugify(u.props.name) === params.itemIndex);
                } catch (err) {
                    console.error(err);
                }
                if (findItemIndex === -1) {
